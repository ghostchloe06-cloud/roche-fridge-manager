(function() {
  ...
  function renderItemForm() {
    ...
    return [
      '<form class="fm-form" data-form="item">',
        ...
        '</div>',
        '<input name="lowStockAt" type="number" min="0" step="0.1" value="' + escapeHtml(item.lowStockAt || "") + '" placeholder="低库存提醒数量（可留空）">',
  ...
  function installStyle() {
    ...
    style.textContent = [
      ...
      "." + ROOT_CLASS + " .fm-grid{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}",
      "." + ROOT_CLASS + " .fm-menu-panel{grid-column:auto}",
      "." + ROOT_CLASS + " .fm-item-groups{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:0 32px}",
      "}",
